...
public class Symbols {
  ...
  private static final Symbol shell = new Symbol(null, 0, false);
  private final Compiler compiler = new Compiler();
  private final Hashtable<Symbol, Symbol> symbols = new Hashtable<>();
  private HashSet<Symbol> delta = null;
  private final Stack<HashSet<Symbol>> frames = new Stack<>();
   ...
   */
  public Symbols() {
    for (Symbol symbol : builtin) {
      add(symbol);
    }
    try {
      for (String define : defines) {
        define(compileWithName(define));
      }
  ...
  void add(Symbol s) {
    Symbol previous = symbols.put(s, s);
  ...
  synchronized Symbol lookup(String name, int arity) {
    return symbols.get(shell.setKey(name, arity));
  }
